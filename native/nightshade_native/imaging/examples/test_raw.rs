use nightshade_imaging::{read_image, read_raw};
use std::path::Path;
...
fn main() {
    ...
    let test_file = std::env::args().nth(1).unwrap_or_else(|| {
        eprintln!(
            "Usage: cargo run --package nightshade_imaging --example test_raw <path_to_raf_file>"
        );
        eprintln!("\nExample:");
        eprintln!(
            "  cargo run --package nightshade_imaging --example test_raw C:\\path\\to\\image.raf"
        );
        std::process::exit(1);
    });
