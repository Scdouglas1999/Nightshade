...
// SAFETY: `frb_generated.rs` is fully auto-generated by flutter_rust_bridge_codegen
// and is overwritten on every codegen run, so we cannot hand-add `// SAFETY:`
// comments inside it. The generated unsafe blocks are FRB-internal Dart-Rust wire
// glue (CstDecode FFI buffer reads, IntoIntoDart pointer transfers) whose safety is
// the responsibility of the flutter_rust_bridge runtime invariants, not our
// hand-written code. We therefore allow the lint at the module boundary; any
// hand-written unsafe in this crate lives in the sibling modules and is gated by
// the workspace-level `-D clippy::undocumented_unsafe_blocks` in CI.
