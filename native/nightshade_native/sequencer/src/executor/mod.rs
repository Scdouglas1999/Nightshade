...
impl SequenceExecutor {
    ...
    /// canonical home of pushed data) so the dashboard sees the same value
    /// the evaluator does.
    ...
    /// Wave 5 Agent 4 — push the latest cloud-motion analyzer reading into
    /// the executor. The cloud-aware triggers (`CloudArrivingIn`,
    /// `CloudOpeningIn`, `CloudCoverThreshold`) read these values on
    /// their next evaluation tick (1 Hz). When the executor is not
    /// running this is a no-op — there is no command channel to send
    /// through; the next `start()` rebuilds an empty snapshot which will
    /// be filled by the Dart side as soon as imaging begins.
    ///
    /// `predicted_clear_sky_alt` and `predicted_clear_sky_az` must be
    /// either both `Some` or both `None`; a half-specified direction is
    /// silently ignored (the executor refuses to invent a missing
    /// coordinate — CLAUDE.md).
    ...
    ///
    /// Map shape: `target_id` → `filter_name` → `seconds`. Repeated calls
    /// merge (last-write-wins per target_id) so the Dart side can stage
    /// each target independently as the handoff dialog cycles through
    /// them. An empty inner map zeroes that target's carry-over (Restart
    /// semantics); omitting the target entirely preserves the existing
    /// behaviour (Continue New / no decision).
    ///
    ...
    /// Pack G — update the global default image-grading thresholds. `None`
    /// disables grading globally (per-node `quality_check` on TakeExposure
    /// still wins). When a sequence is running the change is forwarded as
    /// an `ExecutorCommand::UpdateDefaultQualityCheck` so the live tree
    /// picks it up on the next exposure; otherwise it is recorded for the
    /// next `start()`.
    ...
    /// Wave 7 Agent 3 — update the per-frame defect map applied to lights
    /// during capture. `None` disables defect correction (the user
    /// toggled the apply switch off, or the camera disconnected). A
    /// `Some(...)` value carries the pre-loaded `DefectMap`, replacement
    /// method, kernel size, and the save-original flag.
    ///
    /// When a sequence is running the change is forwarded via
    /// `ExecutorCommand::UpdateDefectMap` so the live capture path sees
    /// it on the next exposure; otherwise it is only recorded for the
    /// next `start()`.
    ...
    /// the next FITS save stamps real keywords (OBSERVER, TELESCOP,
    /// FOCALLEN, APTDIA, INSTRUME, SITEELEV).
    ...
    /// Wave 5 Agent 2 — push the latest sky-brightness reading to a
    /// running executor. The next exposure-burst's adaptive-exposure
    /// decision reads this value from the shared `ExecutionContext`
    /// field. Idle executors (loaded but not running) accept the call
    /// as a no-op — the value is only relevant once an exposure is
    /// being captured.
    ...
    /// Wave 8 — push the composite sky-conditions score that the
    /// `TargetScheduler`'s adaptive-swap logic consults. The Dart side
    /// (`AdaptiveSwapService`) composes the score from transparency /
    /// seeing / cloud / wind every ~30 s and sends this update. When the
    /// executor is idle the value is stashed onto the shared
    /// `ExecutionContext` slot so a subsequent `start()` sees the latest
    /// reading immediately; when running, the command channel routes it
    /// through `UpdateConditionsScore`. Passing `None` clears the slot
    /// (telemetry lost).
    ...
    /// adaptive-swap accounting (last swap, current tier, hysteresis
    /// countdown) for the Run Dashboard "Adaptive Conditions" panel.
    /// Returns `None` only if neither slot has been touched, which lets
    /// the panel render an "off" state cleanly instead of fabricating
    /// zeros.
    ...
    /// adaptive-exposure config. Per-node `ExposureConfig.adaptive_exposure`
    /// still wins; this is the runtime fallback applied to nodes that
    /// don't carry their own block. When a sequence is running the
    /// change is forwarded as an `ExecutorCommand::UpdateDefaultAdaptiveExposure`;
