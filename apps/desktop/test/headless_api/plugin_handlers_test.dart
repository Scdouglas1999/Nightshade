...
void main() {
  ...
  group('PluginHandlers', () {
    ...
    test(
      'upload with matching declared sha256 succeeds, mismatch is rejected',
      () async {
        final bytes = _archive({
    ...
    test('enable then disable round-trips the flag', () async {
      ...
      final disableResponse = await translateHandlerErrors(
        handlers.handleDisablePlugin(
          Request(
            'POST',
            Uri.parse(
              'http://localhost/api/plugins/com.nightshade.flag/disable',
            ),
          ),
          'com.nightshade.flag',
        ),
      ...
      final enableResponse = await translateHandlerErrors(
        handlers.handleEnablePlugin(
          Request(
            'POST',
            Uri.parse(
              'http://localhost/api/plugins/com.nightshade.flag/enable',
            ),
          ),
          'com.nightshade.flag',
        ),
      ...
      final enableBody = jsonDecode(await enableResponse.readAsString()) as Map;
      ...
      final response = await translateHandlerErrors(
        handlers.handleUploadPlugin(
          Request(
            ...
            body: _archive({'id': 'x', 'name': 'x', 'version': '1.0.0'}),
          ),
        ),
      );
      expect(response.statusCode, HttpStatus.badRequest);
