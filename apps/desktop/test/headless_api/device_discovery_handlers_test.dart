  ...
  group('DeviceDiscoveryHandlers.handleGetDevices', () {
    ...
    setUp(() async {
      loggerTempDir = await Directory.systemTemp
          .createTemp('ns_device_discovery_handlers_test_');
    ...
    test('sweeps every device type exactly once and merges results', () async {
      ...
      final response = await translateHandlerErrors(handlers.handleGetDevices(
      ...
      for (final type in DeviceType.values) {
        expect(backend.callCounts[type], 1,
            reason: '${type.name} should be swept exactly once');
      }
    });

    test('dedupes a device reported under multiple type sweeps by id',
        () async {
      ...
      expect(ids.where((id) => id == 'shared-1').length, 1,
    ...
    test('per-type failure surfaces in discoveryErrors without dropping '
      ...
      final ids =
          (body['devices'] as List).cast<Map>().map((d) => d['id']).toSet();
      expect(ids, {'cam-1'});
      ...
      expect(errors[DeviceType.filterWheel.name],
          contains('TouptekSdk DLL missing'));
      expect(
          errors[DeviceType.dome.name], contains('INDI server unreachable'));
      expect(errors.length, 2);
    });

    test('honors explicit deviceType filter (single type, no fan-out)',
        () async {
      final backend = _FakeDeviceBackend(
        devicesByType: {
          DeviceType.camera: [_device('cam-1', DeviceType.camera)],
