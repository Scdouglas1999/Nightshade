...
void main() {
  group('SequencerHandlers', () {
    ...
    test('skip-to-node rejects missing nodeId with 400 JSON error', () async {
      ...
      final response =
          await translateHandlerErrors(handlers.handleSequencerSkipToNode(
        Request(
          'POST',
    ...
    test(
        'skip-to-node with valid nodeId fails through to backend '
    ...
    test(
        'update-autofocus-interval rejects missing everyNFrames with 400 JSON '
        'error', () async {
      final response = await translateHandlerErrors(
          handlers.handleSequencerUpdateAutofocusInterval(
        Request(
          'POST',
          Uri.parse('http://localhost/api/sequencer/update-autofocus-interval'),
          body: jsonEncode({}),
        ),
      ));
    ...
    test(
        'update-autofocus-interval rejects everyNFrames < 1 with 400 JSON '
        'error', () async {
    ...
    test('update-autofocus-interval with valid frames defers to backend',
        () async {
      final response = await translateHandlerErrors(
          handlers.handleSequencerUpdateAutofocusInterval(
        Request(
    ...
    // =====================================================================
    // Wave 4.5 Recovery Mode HTTP handlers
    // =====================================================================

    test('recovery/try-now defers to backend (DisconnectedBackend raises)',
        () async {
      final response =
          await translateHandlerErrors(handlers.handleSequencerRecoveryTryNow(
        Request(
          'POST',
    ...
    test('recovery/abort defers to backend (DisconnectedBackend raises)',
        () async {
      final response =
          await translateHandlerErrors(handlers.handleSequencerRecoveryAbort(
        Request(
          'POST',
    ...
    test('recovery/update-config rejects missing required fields with 400 JSON',
        () async {
      final response = await translateHandlerErrors(
    ...
    test('update-conditions-score rejects non-object score with 400 JSON',
        () async {
      final response = await translateHandlerErrors(
    ...
    test('update-conditions-score accepts null score and defers to backend',
        () async {
      final response = await translateHandlerErrors(
        handlers.handleSequencerUpdateConditionsScore(
          Request(
