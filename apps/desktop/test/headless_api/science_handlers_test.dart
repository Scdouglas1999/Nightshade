...
void main() {
  group('ScienceHandlers', () {
    ...
    test('invalid session bundle ID returns JSON internal error', () async {
      final response = await handlers.handleGetSessionBundle(
        Request(
          'GET',
    ...
    test('compute transform without filter returns JSON bad request', () async {
      final response = await handlers.handleComputePhotometricTransform(
        Request(
          'POST',
          Uri.parse(
      ...
      expect(response.headers['content-type'], 'application/json');
      final body = jsonDecode(await response.readAsString()) as Map;
      expect(body['error'], 'Filter name is required');
    });
