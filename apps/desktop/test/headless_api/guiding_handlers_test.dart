...
void main() {
  group('GuidingHandlers', () {
    ...
    test('algo param names validates axis with JSON bad request', () async {
      final response = await handlers.handlePhd2GetAlgoParamNames(
        Request(
          'GET',
    ...
    test('set algo param validates required value with JSON bad request',
        () async {
      final response = await handlers.handlePhd2SetAlgoParam(
        Request(
          'POST',
      ...
      expect(response.headers['content-type'], 'application/json');
      final body = jsonDecode(await response.readAsString()) as Map;
      expect(body['error'], "Missing 'value' parameter.");
    });

    test('guider lock position validates missing coordinates as JSON',
        () async {
      final response = await handlers.handleGuiderSetLockPosition(
        Request(
          'POST',
      ...
      expect(response.headers['content-type'], 'application/json');
      final body = jsonDecode(await response.readAsString()) as Map;
      expect(body['error'], "Missing 'x' or 'y' parameter.");
    });

    test('disconnected backend failures return JSON internal server error',
        () async {
      final response = await handlers.handlePhd2GetStatus(
