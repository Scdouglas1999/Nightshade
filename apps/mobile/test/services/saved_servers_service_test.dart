...
void main() {
  ...
  group('SavedServer JSON', () {
    test('round-trips non-secret fields without including the auth token',
      ...
      expect(reloaded.lastConnectedAt,
          DateTime.parse('2026-05-25T22:30:00Z'));
  ...
  group('SavedServersService persistence', () {
    test('add then loadAll surfaces the entry, token round-trips secure storage',
        () async {
      ...
      expect(all.first.authToken, isNull,
    ...
    test('upsert by host:port replaces the existing row rather than appending',
        () async {
      final service = SavedServersService(random: fixedRandom());
    ...
    test('removeById drops the row and wipes its secure-storage token',
        () async {
      final service = SavedServersService(random: fixedRandom());
      ...
      expect(await service.tokenFor(a.id), isNull,
    ...
    test('toDiscoveredServer composes the legacy DiscoveredServer shape',
        () async {
      final service = SavedServersService(random: fixedRandom());
      final row = await service.add(
  ...
  group('SavedServersService corruption handling', () {
    test('non-JSON blob is dropped (logged) and loadAll returns empty',
        () async {
      SharedPreferences.setMockInitialValues(<String, Object>{
      ...
      await service.add(
          displayName: 'Fresh', host: '10.0.0.1', port: 8080);
      expect(await service.loadAll(), hasLength(1));
    });

    test('malformed individual rows are skipped but valid ones survive',
        () async {
      final goodRow = {
        'id': 'good-1',
        'displayName': 'Good',
  ...
  group('SavedServersService legacy migration', () {
    test('with no legacy server, migration is a no-op and sets the latch',
        () async {
      final service = SavedServersService(random: fixedRandom());
      final all = await service.loadAll();
      expect(all, isEmpty);
    ...
    test('legacy migration latch prevents re-import across loadAll calls',
        () async {
