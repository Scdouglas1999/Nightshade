...
const _excludedFileHeaderFragments = [
...
const _modularizedSourceFamilyReasons = {
  'apps/desktop/lib/headless_api_server.dart':
      'Facade delegates route setup, lifecycle, event forwarding, middleware, and websocket behavior to focused modules.',
  'packages/nightshade_core/lib/src/backend/network_backend.dart':
      'Facade delegates transport, event tracking, endpoint domains, and wire models to focused modules.',
  'packages/nightshade_core/lib/src/backend/ffi_backend.dart':
      'Facade delegates native hardware workflows and bridge model mapping to focused modules.',
  'packages/nightshade_core/lib/src/providers/sequence_provider.dart':
      'Facade delegates progress reduction and selection clipboard state to focused modules.',
  'packages/nightshade_core/lib/src/services/device_service.dart':
      'Facade delegates cross-device orchestration to focused device-domain modules.',
};
...
Future<void> main(List<String> args) async {
  ...
  final report = {
    ...
    'excludedFileHeaderFragments': _excludedFileHeaderFragments,
    ...
    'modularizedSourceFamilyCount': modularizedSourceFamilies.length,
...
List<File> _dartFilesUnder(Directory repoRoot, Directory directory) {
  ...
  for (final entity in directory.listSync(
    ...
    if (_shouldSkipFile(entity, normalized)) {
      continue;
    }
    files.add(entity);
  }
...
bool _shouldSkipFile(File file, String normalizedPath) {
  if (_shouldSkipPath(normalizedPath)) {
    return true;
  }
  try {
    final handle = file.openSync();
    try {
      final header = utf8.decode(handle.readSync(512), allowMalformed: true);
      return _excludedFileHeaderFragments.any(header.contains);
    } finally {
      handle.closeSync();
    }
...
String _renderMarkdown({
  ...
  required List<Map<String, Object?>> modularizedSourceFamilies,
}) {
  ...
  buffer
    ..writeln()
    ..writeln('## Modularized Source Families')
    ..writeln()
    ..writeln('| Facade lines | Path | Structure |')
    ..writeln('| ---: | --- | --- |');
  if (modularizedSourceFamilies.isEmpty) {
    buffer.writeln('| 0 | None | None |');
  } else {
    for (final entry in modularizedSourceFamilies) {
      buffer.writeln(
        '| ${entry['lineCount']} | `${entry['path']}` | ${entry['reason']} |',
      );
    }
