...
void main() {
  ...
  group('FramingNotifier.pan re-fetch on margin overflow', () {
    ...
    test('a small pan stays a display transform (no re-fetch / no recenter)',
        () async {
      const raHours = 5.0;
    ...
    test('RA recenter step shrinks near the pole (cos(dec) scaling)', () async {
      ...
      expect(stepHighDec, closeTo(stepLowDec / math.cos(60 * math.pi / 180), 1e-4));
    });

    test(
        'threshold uses the DRAWN canvas width, not the native image pixel '
        'width (small cutout on a large canvas does not over-trigger)',
        () async {
      ...
      expect(state.panX, 50,
          reason: 'pan must accumulate as a display transform; drawn-width '
              'mapping keeps 50px well under the re-fetch threshold');
      expect(state.target!.raHours, raHours,
          reason: 'no recenter: drawn px-per-degree (720/4=180) means 50px is '
              'only 0.28deg, not the 2.0deg the native-pixel math wrongly gave');
    });

    test(
        'threshold scales with zoom (a zoomed-in view re-centers on a smaller '
        'pan)', () async {
