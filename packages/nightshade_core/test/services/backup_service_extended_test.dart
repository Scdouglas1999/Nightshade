  ...
  group('BackupService P2-9 extended coverage', () {
    ...
    test(
      ...
      expect(backupResult.success, isTrue,
          reason: backupResult.errorMessage);
      expect(await backupFile.exists(), isTrue);

      // Restore into a fresh DB.
      final restore = await serviceFor(dstDb).restoreBackup(
    ...
    test('second restore is idempotent (no row duplication)', () async {
      ...
      final backupFile = File(p.join(tempDir.path, 'backup.nsbackup'));
      final backupResult =
          await serviceFor(srcDb).createBackup(customPath: backupFile.path);
      expect(backupResult.success, isTrue,
          reason: backupResult.errorMessage);

      // First restore populates dst.
      final first = await serviceFor(dstDb).restoreBackup(
        filePath: backupFile.path,
      );
      ...
      final second = await serviceFor(dstDb).restoreBackup(
        filePath: backupFile.path,
      );
