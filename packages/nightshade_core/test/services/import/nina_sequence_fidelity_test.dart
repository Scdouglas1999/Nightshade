...
void main() {
  group('NINA fidelity (lossless cases)', () {
    test(
        ...
        () async {
      final result = await _importFixture('nina_advanced.json',
    ...
    test('target header: TargetName and InputCoordinates round-trip exactly',
        () async {
      final result = await _importFixture('nina_advanced.json',
    ...
    test('loop iterations from Conditions[].LoopCondition.Iterations '
      ...
      final result = await _importFixture('nina_advanced.json',
          sequenceName: 'fidelity');
      final loops =
          result.sequence.nodes.values.whereType<LoopNode>().toList();
      ...
      expect(loops.first.repeatCount, 30,
    ...
    test('meridian-flip Trigger lifts into the child list with '
      ...
      final result = await _importFixture('nina_advanced.json',
          sequenceName: 'fidelity');
      final flips =
          result.sequence.nodes.values.whereType<MeridianFlipNode>().toList();
      expect(flips, hasLength(1));
      expect(flips.first.minutesPastMeridian, 7.0);
      expect(flips.first.useGlobalDefaults, isFalse,
    ...
    test('parent-target loop count from Conditions[].Iterations folds onto '
      ...
      final result = await _importFixture('nina_basic.json',
          sequenceName: 'M31');
      ...
      expect(loops, isNot(contains(10)),
          reason:
  ...
  group('NINA fidelity (lossy / dropped cases)', () {
    test('plugin script node, SafetyMonitor trigger, and SmartExposure '
      ...
      expect(
        ...
        throwsA(isA<UnsupportedNodeError>().having(
    ...
    test('force-import: ALL unsupported entries are surfaced individually '
      ...
      final result = await _importFixture('nina_unsupported_edge.json',
          forceUnsupported: true, sequenceName: 'edge forced');
      ...
      final unsupportedTypes =
          result.unsupportedNodes.map((u) => u.sourceType).toSet();
      expect(unsupportedTypes, containsAll([
        'CustomScriptNode',
        'SafetyMonitorTrigger',
        'SmartExposure',
        'SwitchFilter',
      ...
      final unsupportedDrops = result.droppedNodes
          ...
          .map((d) => d.sourceType)
          .toSet();
      expect(unsupportedDrops, containsAll([
        'CustomScriptNode',
        'SafetyMonitorTrigger',
        'SmartExposure',
        'SwitchFilter',
    ...
    test('Annotation node is dropped silently with DropReason.decorative '
      ...
      final result = await _importFixture('nina_unsupported_edge.json',
          forceUnsupported: true, sequenceName: 'edge');
    ...
    test('Enabled: false leaf is dropped with DropReason.disabled', () async {
      ...
      // ExposureNode list.
      final ondisk =
          result.sequence.nodes.values.whereType<ExposureNode>().toList();
    ...
    test('NINA script triggers, plugin nodes, and dome plugins are NOT '
      ...
      final result = await _importFixture('nina_unsupported_edge.json',
          forceUnsupported: true, sequenceName: 'edge');
      final actuallyUnsupported =
          result.unsupportedNodes.map((u) => u.sourceType).toSet();
      expect(actuallyUnsupported.containsAll(unsupportedSet), isTrue,
          reason:
    ...
    test('SwitchFilter with no resolvable filter name is reported '
        'unsupported (Nightshade requires a non-empty filterName)',
        () async {
      final result = await _importFixture('nina_unsupported_edge.json',
          forceUnsupported: true, sequenceName: 'edge');
      ...
      expect(brokenFilter.reason, contains('Nightshade'),
          reason:
  ...
  group('NINA fidelity (complex flow control)', () {
    test('multiple DSO containers in a single SequenceRoot all survive '
      ...
      final result = await _importFixture('nina_complex_flow.json',
    ...
    test('ParallelContainer round-trips as ParallelNode', () async {
      final result = await _importFixture('nina_complex_flow.json',
          forceUnsupported: true, sequenceName: 'two-target');
      final parallels =
          result.sequence.nodes.values.whereType<ParallelNode>().toList();
      expect(parallels, hasLength(1),
  ...
  group('SGP fidelity (lossless cases)', () {
    test('SGP basic two-target fixture: every event becomes a '
      ...
      final result = await _importFixture('sgp_basic.sgf',
      ...
      final filters =
          result.sequence.nodes.values.whereType<FilterChangeNode>().toList();
...
Future<ImportResult> _importFixture(
  ...
  final content =
      await File('test/services/import/fixtures/$fixtureFile').readAsString();
