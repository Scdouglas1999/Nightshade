...
void main() {
  group('NightAnalysisService detectors (pure analyze)', () {
    ...
    test('focus-drift stays silent on flat HFR (noise only)', () {
      final data = NightData(_series(
        count: 16,
        hfr: (i) => 2.0 + (i.isEven ? 0.02 : -0.02),
      ));
      final report = svc.analyze(data, sessionId: 1);
      expect(_finding(report, 'focus_drift'), isNull);
    });

    test('guiding-correlation fires when RMS spikes coincide with elongation',
        () {
    ...
    test('guiding-correlation silent when RMS data present but no elongation',
        () {
      final data = NightData(_series(
        count: 14,
        guidingRmsTotal: (i) => (i == 7 || i == 8) ? 2.0 : 0.5,
    ...
    test('dew/HFR-collapse fires on a sharp irreversible HFR step + star loss',
        () {
      ...
      expect(f.evidenceSubIds, contains(6)); // first post-collapse sub (index5)
    ...
    test('cloud-step ignores a single isolated dropout (needs >= 2 in a row)',
        () {
      final data = NightData(_series(
        count: 16,
    ...
    test(
        'cloud-step does NOT switch to SNR mode on partial coverage '
    ...
    test(
        'cloud-step uses SNR mode only with near-complete coverage and fires '
        'on a real run', () {
  ...
  group('NightAnalysisService scoring', () {
    test('score is monotone non-increasing as severity worsens', () {
      ...
      final warn = svc.analyze(NightData(_series(
        count: 16,
        hfr: (i) => 2.0 + i * 0.06, // early≈2.21 → late≈2.69, ~22% rise
      )));

      // A critical dew collapse.
      final critical = svc.analyze(NightData(_series(
        count: 12,
        hfr: (i) => i < 5 ? 2.0 : 4.5,
        starCount: (i) => i < 5 ? 1500 : 500,
      )));
    ...
    test('findings are ranked most-severe first and headline reflects worst',
        () {
      final svc = _pureService();
  ...
  group('NightAnalysisService DB-bound computeReport', () {
    ...
    Future<void> insertSub({
      ...
      await db.into(db.capturedImages).insert(
            CapturedImagesCompanion.insert(
              ...
              capturedAt:
                  DateTime.utc(2026, 6, 7, 22).add(Duration(minutes: 2 * index)),
    ...
    test('computeReport reads subs, detects, and persists a night_reports row',
        () async {
    ...
    test('computeReport on a session with no lights yields a clean report',
        () async {
      final sessionId = await insertSession();
      final report = await svc.computeReport(sessionId: sessionId);
      expect(report.findings, isEmpty);
    ...
    test(
        'defaulted-0.0 science snr rows do NOT fire a false cloud storm',
        () async {
      ...
      for (var i = 0; i < 12; i++) {
        await insertSub(
            sessionId: sessionId, index: i, hfr: 2.0, starCount: 1500);
        final id = await (db.select(db.capturedImages)
      ...
      for (var i = 0; i < imageIds.length; i++) {
        await db.into(db.scienceFrameQualityMetrics).insert(
