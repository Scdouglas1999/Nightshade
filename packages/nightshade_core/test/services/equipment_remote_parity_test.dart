...
void main() {
  ...
  group('equipment remote parity', () {
    test('connectActiveProfile uses host active profile on NetworkBackend',
        () async {
    ...
    test('updateProfileDevices saves through host API on NetworkBackend',
        () async {
      final backend = _MockNetworkBackend();
      when(() => backend.eventStream).thenAnswer(
        (_) => const Stream<NightshadeEvent>.empty(),
      ...
      final captured = verify(() => backend.saveProfile(captureAny())).captured;
