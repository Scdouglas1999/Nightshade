...
void main() {
  ...
  Future<int> insertDark({
    ...
    return darkDao.addEntry(DarkLibraryCompanion.insert(
      filePath: '/raw/dark_${exposureTime.toInt()}_${gain}_'
  ...
  Future<int> insertFlat({
    ...
    return flatDao.addEntry(
      filePath: '/master/flat_${filter}_'
          '${DateTime.now().microsecondsSinceEpoch}.fits',
      filter: filter,
      gain: gain,
      offset: offset,
  ...
  group('dark matching', () {
    test('requires exact gain/offset/binning', () async {
      ...
      final wrongGain = await service.match(const LightFrameContext(
        gain: 200,
        offset: 50,
        exposureSeconds: 300,
      ));
      expect(wrongGain.dark, isNull,
          reason: 'a dark at gain 100 must not match a gain-200 light');
      expect(wrongGain.warnings, isNotEmpty);

      final wrongOffset = await service.match(const LightFrameContext(
        gain: 100,
        offset: 99,
        exposureSeconds: 300,
      ));
      ...
      final wrongBin = await service.match(const LightFrameContext(
        gain: 100,
        offset: 50,
        exposureSeconds: 300,
        binX: 2,
      ...
      final exact = await service.match(const LightFrameContext(
        gain: 100,
        offset: 50,
        exposureSeconds: 300,
      ));
    ...
    test('exact gain/offset/exposure/temp scores a perfect 100', () async {
      await insertDark(
          exposureTime: 300, gain: 100, offset: 50, temperature: -10);

      final match = await service.match(const LightFrameContext(
        gain: 100,
        offset: 50,
        exposureSeconds: 300,
        temperature: -10,
    ...
    test('temperature delta beyond tolerance produces a warning', () async {
      ...
      expect(
          match.dark!.warnings
              .any((w) => w.toLowerCase().contains('temperature')),
          isTrue);
      expect(match.dark!.score, lessThan(100));
    });
    ...
    test('dark older than the staleness window warns', () async {
      ...
      expect(match.dark, isNotNull);
      expect(
          match.dark!.warnings.any((w) => w.contains('120 days')), isTrue);
    });
  });
  ...
  group('bias matching', () {
    test('matches exact gain/offset and is detected as a bias', () async {
      await insertDark(
          exposureTime: 0, gain: 100, offset: 50, frameType: 'bias');
  ...
  group('flat matching', () {
    test('matches by filter and prefers the newest', () async {
      await insertFlat(
          filter: 'L', createdAt: now.subtract(const Duration(days: 2)));
      final newerId = await insertFlat(
          filter: 'L', createdAt: now.subtract(const Duration(days: 1)));
      ...
      expect(match.flat!.record.id, newerId,
          reason: 'newest qualifying flat wins');
    });

    test('flat older than 7 days warns (stale)', () async {
      await insertFlat(
          filter: 'L', createdAt: now.subtract(const Duration(days: 10)));
  ...
  group('tags + listing', () {
    test('setTags / setNotes round-trip onto the listed record', () async {
      ...
      await service.setTags(
          CalibrationMasterType.dark, id, ['keeper', ' best ']);
      await service.setNotes(CalibrationMasterType.dark, id, 'shot under -10C');

      final records =
          await service.listMasters(enrichFromHeaders: false);
  ...
  group('deletion', () {
    test('deleteMaster removes the row and its tags', () async {
      ...
      final removed = await service.deleteMaster(CalibrationMasterType.dark, id);
      expect(removed, isTrue);

      final records = await service.listMasters(enrichFromHeaders: false);
      expect(records.where((r) => r.id == id), isEmpty);
      expect(await tagsDao.getForMaster(CalibrationMasterType.dark, id),
          isNull);
    });

    test('deleting a non-existent master returns false', () async {
      expect(
          await service.deleteMaster(CalibrationMasterType.dark, 9999), isFalse);
    });
  });
}
