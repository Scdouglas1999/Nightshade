...
void main() {
  ...
  test('routes PluginNodeRequested → dispatcher → sequencerPluginNodeFinished',
      () async {
    ...
    when(() => backend.sequencerPluginNodeFinished(
    ...
    for (var i = 0; i < 40; i++) {
      ...
      try {
        verify(() => backend.sequencerPluginNodeFinished(
        ...
        expect(
            capturedRequest!.configJson, '{"title":"E2E","message":"Hello"}');
  ...
  test('dispatcher throw is caught and surfaced as a failure verdict',
      () async {
    final dispatcher = (PluginNodeDispatchRequest request) async {
  ...
  test('network/remote backends leave plugin dispatch to the host', () async {
    ...
    verifyNever(() => backend.sequencerPluginNodeFinished(
          nodeId: any(named: 'nodeId'),
          success: any(named: 'success'),
          message: any(named: 'message'),
          structuredDetailJson: any(named: 'structuredDetailJson'),
