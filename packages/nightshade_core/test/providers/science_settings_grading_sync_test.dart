...
void main() {
  ...
  test('auto frame grading toggles the sequencer image grading default',
      () async {
    ...
    expect(await db.settingsDao.getSetting('science.grading.auto_enabled'),
        'true');
    expect(await db.settingsDao.getSetting('image_grading_enabled'), 'true');
  });

  test('science settings reflect an existing sequencer image grading default',
      () async {
    final db = NightshadeDatabase.forTesting(NativeDatabase.memory());
