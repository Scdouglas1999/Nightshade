  ...
  group('profileConnectionStatusProvider', () {
    ...
    test('unassigned + nothing-connected profile has empty rollups', () {
      final status = container
          .read(profileConnectionStatusProvider(profile()));
    ...
    test('a connected matching device rolls up into the counts', () {
      container.read(cameraStateProvider.notifier).state =
      ...
      final status = container.read(
          profileConnectionStatusProvider(profile(cameraId: 'native:zwo:asi2600:0')));

      expect(status.coreTotalCount, 1);
      expect(status.coreConnectedCount, 1);
      expect(status.hasConnectedCore, isTrue);
      expect(status.hasDisconnectedCore, isFalse);
      expect(status[ProfileDeviceSlot.camera].profileState,
    ...
    test('PHD2 guider counts via canonical id (5/5 with all connected)', () {
      ...
      container.read(filterWheelStateProvider.notifier).state =
      ...
      expect(status[ProfileDeviceSlot.guider].profileState,
          DeviceConnectionState.connected);
    ...
    test('a different connected device is flagged as a mismatch', () {
      ...
      final status = container.read(profileConnectionStatusProvider(
    ...
    test('connected-but-unmatched still offers Connect All (disconnected core)',
        () {
