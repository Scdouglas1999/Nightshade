...
void main() {
  group('NetworkBackend WebSocket heartbeat', () {
    ...
    test('parses HostStateChanged system events from headless wrapper',
        () async {
      final server = await _startServer((socket) {
        Timer(const Duration(milliseconds: 50), () {
          socket.add(jsonEncode({
    ...
    test(
      'P2-13: first attempt emits `connecting`, post-success drop emits '
      '`reconnecting`',
      () async {
        ...
        try {
          ...
          expect(
            ...
            reason:
                'P2-13: first connect attempt must use the connecting state',
          );
    ...
    test('preserves structured sequencer progress payloads over WebSocket',
        () async {
      final server = await _startServer((socket) {
        Timer(const Duration(milliseconds: 50), () {
          socket.add(jsonEncode({
      ...
      try {
        ...
        final detail = jsonDecode(event.data['detail_json'] as String)
  ...
  server.listen((request) async {
    ...
    if (request.uri.path == '/api/devices') {
      final selectedDevicesBody = devicesBodies == null
          ? devicesBody
          : devicesBodies[
              devicesRequestCount.clamp(0, devicesBodies.length - 1)];
      devicesRequestCount++;
      request.response
        ..statusCode = HttpStatus.ok
        ..headers.contentType = ContentType.json
