...
void main() {
  group('NetworkBackend DepthLock contract', () {
    ...
    test('mutations send the definition and the expected revision', () async {
      ...
      expect(
        fake.requests.last.url.queryParameters['expectedRevision'],
        '3',
      );
    ...
    test('a created goal may leave its id to the host', () async {
      ...
      await backend.createDepthLockGoal(
        DepthLockGoalDefinition.fromJson(Map<String, dynamic>.from(_definition)),
      );
    ...
    test('ingest, inspection and the geometry helpers decode', () async {
      ...
      expect(
        await backend.checkDepthLockMeasurement(
          DepthLockMeasurement.fromJson(Map<String, dynamic>.from(_measurement)),
        ),
        2400,
      );
    });

    test('a report with no forecast yet decodes as absent, not as zeros', () async {
    ...
    test('a floor suggestion posts the paths and decodes the measurement', () async {
      final fake = FakeNetworkClient()
        ..setResponse(
    ...
    test('a missing envelope key throws rather than reading as empty', () async {
      final fake = FakeNetworkClient()
    ...
    test('a stale revision and a missing goal keep their native reason', () async {
      final fake = FakeNetworkClient()
        ..setResponse(
          '/api/depthlock/goals/m42-ha',
          method: 'PUT',
