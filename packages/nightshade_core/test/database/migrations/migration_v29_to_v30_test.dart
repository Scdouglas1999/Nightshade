...
void main() {
  group('stepped migration onUpgrade v29 -> v30 (guide_rms_history)', () {
    ...
    test('exposure_seconds becomes nullable after upgrade', () async {
      ...
      try {
        ...
        expect(
          ...
          equals(0),
          reason: 'exposure_seconds must be nullable after v30 upgrade '
    ...
    test('idx_guide_rms_mount_recent is created during upgrade', () async {
      ...
      try {
        ...
        expect(rows, hasLength(1),
            reason: 'idx_guide_rms_mount_recent must exist after v30 '
    ...
    test('preserves existing guide_rms_history rows through the recreate',
        () async {
      ...
      try {
        final rows = await upgraded
            ...
            .get();
        expect(rows, hasLength(1),
            reason: 'The seeded v29 row must survive the v30 recreate. '
        ...
        expect((row['total_rms_arcsec'] as num).toDouble(),
            closeTo(0.83, 1e-9));
        expect(row['sample_count'], equals(312));
        expect((row['exposure_seconds'] as num).toDouble(),
            closeTo(90.0, 1e-9));
