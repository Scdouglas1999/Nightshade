...
void main() {
  ...
  setUp(() async {
    ...
    tempDir = await Directory.systemTemp.createTemp('nightshade_filesize_test_');
  });
  ...
  group('insertSequenceFrame fileSize population', () {
    test('populates fileSize when on-disk file exists', () async {
      final file =
          File('${tempDir.path}${Platform.pathSeparator}frame.fits');
    ...
    test('leaves fileSize NULL but still writes the row when file is missing',
        () async {
      final missingPath =
  ...
  group('backfillMissingFileSizes', () {
    test('updates rows where file_size IS NULL and skips missing files',
        () async {
      // One real file (will be updated), one missing (will be skipped),
      // one empty-path (also skipped).
      final realFile =
          File('${tempDir.path}${Platform.pathSeparator}real.fits');
