...
void main() {
  group('migration onUpgrade v42 -> v43 (durable campaigns)', () {
    ...
    test('creates the campaigns table + index on upgrade', () async {
      final tempDir =
          await Directory.systemTemp.createTemp('nightshade_v42_v43_create_');
      addTearDown(() async {
    ...
    test('a campaign round-trips through CampaignsDao after the upgrade',
        () async {
      final tempDir = await Directory.systemTemp
          .createTemp('nightshade_v42_v43_roundtrip_');
      addTearDown(() async {
        if (await tempDir.exists()) await tempDir.delete(recursive: true);
      });
      final dbFile = await createV42Database(tempDir);
    ...
    test('re-running the v43 migration block is idempotent (no-op)', () async {
      final tempDir =
          await Directory.systemTemp.createTemp('nightshade_v42_v43_idem_');
      addTearDown(() async {
        if (await tempDir.exists()) await tempDir.delete(recursive: true);
      });
      final dbFile = await createV42Database(tempDir);
      ...
      await CampaignsDao(first).upsertDesired(
        targetId: targetId,
        filter: 'OIII',
        desiredCount: 30,
      );
    ...
    test('UNIQUE(target_id, filter) enforces one row per pair on upgrade',
        () async {
      final tempDir =
          await Directory.systemTemp.createTemp('nightshade_v42_v43_unique_');
      addTearDown(() async {
        if (await tempDir.exists()) await tempDir.delete(recursive: true);
      });
      final dbFile = await createV42Database(tempDir);
      ...
      try {
        ...
        final firstId =
            await dao.upsertDesired(targetId: targetId, filter: 'L', desiredCount: 50);
        // Re-upserting the same (target, filter) updates the existing row
        // (preserving accepted_count) rather than inserting a duplicate.
        final secondId =
            await dao.upsertDesired(targetId: targetId, filter: 'L', desiredCount: 75);
