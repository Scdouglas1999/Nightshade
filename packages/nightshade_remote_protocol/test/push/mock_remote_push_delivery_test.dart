...
void main() {
  ...
  test('enumerates every registered token across platforms', () async {
    final store = InMemoryPushTokenStore(
      tokens: const [
        RegisteredPushToken(
            deviceId: 'phone-1', platform: 'fcm', token: 'f1'),
        RegisteredPushToken(
            deviceId: 'phone-2', platform: 'fcm', token: 'f2'),
        RegisteredPushToken(
            deviceId: 'phone-3', platform: 'apns', token: 'a3'),
      ],
    );
    ...
    expect(
      mock.delivered.map((d) => d.token.token).toSet(),
      {'f1', 'f2', 'a3'},
    );
  ...
  test('respects the master per-device enabled gate', () async {
    final store = InMemoryPushTokenStore(
      tokens: const [
        RegisteredPushToken(
            deviceId: 'phone-on', platform: 'fcm', token: 'on'),
        RegisteredPushToken(
            deviceId: 'phone-off', platform: 'fcm', token: 'off'),
      ],
      prefs: const {
        'phone-off': DevicePushPreferences(enabled: false),
      },
    );
    final mock = MockRemotePushDelivery(store: store);
  ...
  test('respects a per-category mute flag for the matching category',
      () async {
    final store = InMemoryPushTokenStore(
      tokens: const [
        RegisteredPushToken(
            deviceId: 'tablet', platform: 'fcm', token: 'tab'),
        RegisteredPushToken(
            deviceId: 'pocket', platform: 'fcm', token: 'pkt'),
      ],
      prefs: const {
    ...
    expect(
      mock.delivered.map((d) => d.token.deviceId).toSet(),
      {'tablet', 'pocket'},
    );
  });

  test('a missing prefs row delivers every category (fail-open default)',
      () async {
    final store = InMemoryPushTokenStore(
      tokens: const [
        RegisteredPushToken(
            deviceId: 'new-phone', platform: 'apns', token: 'np'),
      ],
    );
    final mock = MockRemotePushDelivery(store: store);
  ...
  test('an unknown category is never muted (new criticals still reach)',
      () async {
    final store = InMemoryPushTokenStore(
      tokens: const [
        RegisteredPushToken(
  ...
  test('platforms filter restricts the fan-out set', () async {
    final store = InMemoryPushTokenStore(
      tokens: const [
        RegisteredPushToken(
            deviceId: 'android', platform: 'fcm', token: 'f'),
        RegisteredPushToken(
            deviceId: 'ios', platform: 'apns', token: 'a'),
      ],
    );
    // An FCM-only delivery (as the real FcmHttpV1Delivery would be) sees only
    // the Android token.
    final fcmOnly =
        MockRemotePushDelivery(store: store, platforms: const ['fcm']);
  ...
  test('mute gate matches the production wire frame (eventType, not category)',
      () async {
    ...
    final store = InMemoryPushTokenStore(
      ...
      prefs: const {
        'muted': DevicePushPreferences(muteWeatherUnsafe: true),
      },
    );
    final mock = MockRemotePushDelivery(store: store);
