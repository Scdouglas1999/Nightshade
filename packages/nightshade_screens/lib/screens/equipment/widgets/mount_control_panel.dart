...
class _MountControlPanelState extends ConsumerState<MountControlPanel> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (widget.mountState.canSetTrackingRate) ...[
            const SizedBox(height: 12),
            _buildTrackingRateSelector(),
          ],
          const SizedBox(height: 12),
          SizedBox(
  ...
  Widget _buildTrackingRateSelector() {
    return Row(
      children: [
        Icon(
          LucideIcons.gauge,
          ...
          child: Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<TrackingRate>(
                value: widget.mountState.trackingRate,
                isExpanded: true,
                ...
                items: TrackingRate.values.map((rate) {
                  return DropdownMenuItem(
                    value: rate,
                    child: Text(_getTrackingRateLabel(rate)),
                  );
                }).toList(),
                onChanged: (rate) {
                  if (rate != null) {
                    _setTrackingRate(rate);
                  }
                },
              ),
            ),
  ...
  String _getTrackingRateLabel(TrackingRate rate) {
    switch (rate) {
  ...
  Future<void> _setTrackingRate(TrackingRate rate) async {
    try {
      await ref.read(deviceServiceProvider).setMountTrackingRate(rate.index);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to set tracking rate: $e')),
        );
      }
    }
  }
