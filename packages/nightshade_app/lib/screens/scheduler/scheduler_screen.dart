...
import 'widgets/integration_goals_editor.dart';
import 'widgets/target_constraints_editor.dart';
import 'widgets/target_score_row.dart';

/// Main entry point for the RoboTarget-class dynamic scheduler.
///
/// Layout (desktop):
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          if (isMobile) {
            return _buildMobile(context, colors, status, decision, engine);
          }
          return _buildDesktop(context, colors, status, decision, engine);
        },
      ),
    );
  }
    ...
    if (name == null) {
      return Text(
        status.state == SchedulerState.running
            ...
            : 'Press Start to evaluate candidates.',
        ...
        children: [
          ...
          if (decision == null)
            Padding(
              padding: const EdgeInsets.all(NightshadeTokens.spaceLg),
              child: Text(
                'Awaiting first evaluation. Press Start in the panel on the left, '
                'or tap Re-evaluate to compute an initial decision.',
                style:
            ...
            Padding(
              padding: const EdgeInsets.all(NightshadeTokens.spaceLg),
              child: Text(
                'No candidates in the database. Add targets in the Targets table '
                'and assign them integration goals.',
                style:
                    TextStyle(fontSize: 13, color: colors.textSecondary),
              ),
            )
